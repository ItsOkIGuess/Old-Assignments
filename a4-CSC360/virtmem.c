...
struct page_table_entry {
	...
	int lu; /* For LRU Replacement Scheme*/
	int referenced; /*For Second Chance Replacement Scheme*/
};
...
long resolve_address(long logical, int memwrite)
{
    ...
    if (frame != -1) {
		...
		page_table[frame].lu = last_used;
		page_table[frame].referenced = TRUE;
		last_used++;
		if(memwrite == TRUE){
	...
	if(page_replacement_scheme == REPLACE_FIFO){
		...
		page_table[counter % size_of_memory].page_num = page;
		page_table[counter % size_of_memory].free = FALSE;
		
		if(page_table[counter % size_of_memory].dirty == TRUE){
			swap_outs++;
		}
		if(memwrite == TRUE){
			page_table[counter % size_of_memory].dirty = TRUE;
		}else{
			page_table[counter % size_of_memory].dirty = FALSE;
		}
		
		swap_ins++;
		effective = ((counter % size_of_memory) << size_of_frame) | offset;
		counter++;
		return effective;	
	}
	
	if(page_replacement_scheme == REPLACE_LRU){
		int min = page_table[0].lu;
		int min_index = 0;
		for (i = 0; i < size_of_memory;i++){
			if(page_table[i].lu < min){
				min = page_table[i].lu;
				min_index = i;
			}
		}
		if(page_table[min_index].dirty == TRUE){
			swap_outs++;
		}
		if(memwrite == TRUE){
			page_table[min_index].dirty = TRUE;
		}else{
			page_table[min_index].dirty = FALSE;
		}
		page_table[min_index].page_num = page;
		page_table[min_index].free = FALSE;
		page_table[min_index].lu = last_used;
		effective = (min_index << size_of_frame) | offset;
		last_used++;
		swap_ins++;
		return effective;	
	}
	...
	if(page_replacement_scheme == REPLACE_SECONDCHANCE){
		...
		while(page_table[counter % size_of_memory].referenced == TRUE){
			page_table[counter % size_of_memory].referenced = FALSE;
			counter++;
		}
		...
		page_table[counter % size_of_memory].page_num = page;
		page_table[counter % size_of_memory].free = FALSE;
		page_table[counter % size_of_memory].referenced = TRUE;
		swap_ins++;
		effective = ((counter % size_of_memory) << size_of_frame) | offset;
		counter++;
		return effective;	
